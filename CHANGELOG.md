...
* Bugfix: Saving a model whose `SpicyUUIDField`/`TypeIDField` holds a malformed id string now raises `ProgrammingError`, like the other write paths, instead of leaking the internal `MalformedSpicyIdError` from `save()`.
